...
<body>
	...
	<div class="mainindex" id="mainindex">
		<div class="maincon">
			<!--列表信息-->
			<div class="ziliaobox">
				<form action="${ctx}/user/upUser" method="post"
					...
					<table width="100%" class="zltab" border="0" cellspacing="0"
						...
						<tr class="tr1">
							...
							<td width="80%"><input name="id" type="hidden"
								value="${user.id}"> <input name="username" type="text"
								class="dfinput" value="${user.username}" style="width: 85%;"
								placeholder="" /> <span id="username"></span></td>
						</tr>
						<tr class="tr1">
							<td width="20%" style="text-align: right">用户名：</td>
							<td width="80%"><input name="nickname"
								value="${user.nickname}" type="text" class="dfinput"
						...
						<tr class="tr1">
							...
							<td width="80%"><select name="roleIds" style="width: 85%"
								class="select1">
								...
								<select class="select1" name="dept"
									style="width: 85%">
										...
										<c:forEach items="${depts}" var="d">
											<option value="${d.name}"
												<c:if test="${d.name == user.dept}">selected="selected"</c:if>
												>${d.name}</option>
										</c:forEach>
								</select>
							</td>
						...
						<tr class="tr1">
							...
							<td width="80%"><select name="isDel" style="width: 85%"
								class="select1">
									<option value="1"
										<c:if test="${user.isDel == 1}">selected="selected"</c:if>>启用</option>
									<option value="0"
										<c:if test="${user.isDel == 0}">selected="selected"</c:if>>禁用</option>
							</select></td>
						</tr>
						<tr class="tr1">
							<td width="20%"></td>
