			...
			<div class="ziliaobox">
				<table width="100%" class="zltab" border="0" cellspacing="0"
					...
					<tr class="tr0">
						...
						<form action="${ctx}/role/saUs" method="post"
							onsubmit="return checkOut()">
							<tr class="tr1">
								<td width="20%" style="text-align: right">角色名称：</td>
								<td width="80%"><input name="role" value="${role.role}"
									type="text" class="dfinput" style="width: 85%;" placeholder="" />
									<span id="role"></span>
									</td>
							</tr>
							<tr class="tr1">
							...
							<tr class="tr1">
								...
								<td width="80%"><c:if
										...
										<c:forEach items="${deptRole}" var="dept">
											...
											<c:if test="${role.authIds != null}">
												<c:forEach items="${role.authIds}" var="auth">
													<c:if test="${auth == dept.id}">
							...
							<tr class="tr1">
								...
								<td width="80%">
									<!-- <select class="select1" name="isDel" --> 
								<select
